...
public class ChronicleMapCacheConfigTest {
    ...
    public void shouldProvidePersistedFileWhenCacheDirIsConfigured() throws Exception {
        ...
        assertThat(configUnderTest(gerritConfig).getPersistedFile()).isEqualTo(sitePaths.resolve(directory).resolve(String.format("%s.dat", cacheName)).toFile());
    }
    ...
    public void shouldProvideConfiguredDiskLimitWhenDefined()  throws Exception {
        ...
        assertThat(configUnderTest(gerritConfig).getLimit()).isEqualTo(configuredDiskLimit);
    }

    @Test
    public void shouldProvideDefinitionDiskLimitWhenNotCofigured() {
        assertThat(configUnderTest(gerritConfig).getLimit()).isEqualTo(definitionDiskLimit);
    }
    ...
    public void shouldProvideDefaultAverageValueSizeWhenNotConfigured() {
        assertThat(configUnderTest(gerritConfig).getAverageValueSize()).isEqualTo(DEFAULT_AVG_VALUE_SIZE);
    }

    private ChronicleMapCacheConfig configUnderTest(StoredConfig gerritConfig) {
        return new ChronicleMapCacheConfig(gerritConfig, sitePaths, cacheName, cacheKey, definitionDiskLimit);
    }
}
