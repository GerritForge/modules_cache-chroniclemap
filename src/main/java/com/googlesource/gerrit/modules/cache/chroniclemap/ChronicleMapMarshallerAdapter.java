...
public class ChronicleMapMarshallerAdapter<T> implements BytesWriter<T>, BytesReader<T>,
                                                                 ReadResolvable<ChronicleMapMarshallerAdapter<T>> {
    ...
    public T read(Bytes in, T using) {
            using = cacheSerializer.deserialize(in.toByteArray());
            return using;
    }

    @Override
    public void write(Bytes out, T toWrite) {
        final byte[] serialize = cacheSerializer.serialize(toWrite);
        out.write(serialize);
    }
}
